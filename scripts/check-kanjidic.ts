/**
 * Compares stroke_count and radical in our characters table
 * against KANJIDIC2 data and reports mismatches.
 *
 * Usage: npx tsx scripts/check-kanjidic.ts
 */
...
const kanjidicMap = new Map<
  number,
  { strokes: number | null; radical: number | null }
>();
...
async function main() {
  ...
  let strokeMismatches = 0;
  let radicalMismatches = 0;

  for (const row of rows) {
    const ref = kanjidicMap.get(row.id);
    if (!ref) continue;

    if (
      row.stroke_count !== null &&
      ref.strokes !== null &&
      row.stroke_count !== ref.strokes
    ) {
      strokeMismatches++;
      console.log(
        `STROKE:  ${row.literal} (U+${row.id.toString(16).toUpperCase().padStart(4, "0")}) — ours: ${row.stroke_count}, kanjidic: ${ref.strokes}`,
      );
    }
    if (
      row.radical !== null &&
      ref.radical !== null &&
      row.radical !== ref.radical
    ) {
      radicalMismatches++;
      const oursChar = KANGXI_RADICALS[row.radical] ?? "?";
      const refChar = KANGXI_RADICALS[ref.radical] ?? "?";
      console.log(
        `RADICAL: ${row.literal} (U+${row.id.toString(16).toUpperCase().padStart(4, "0")}) — ours: ${row.radical} (${oursChar}), kanjidic: ${ref.radical} (${refChar})`,
      );
    }
  }

  console.log(`\nTotal stroke mismatches:  ${strokeMismatches}`);
  console.log(`Total radical mismatches: ${radicalMismatches}`);
